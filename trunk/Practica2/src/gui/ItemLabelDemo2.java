...
public class ItemLabelDemo2 extends ApplicationFrame {
    ...
    static class LabelGenerator implements CategoryItemLabelGenerator {
		...
		@Override
		public String generateLabel(CategoryDataset dataset, int series, int arg2) {
			String result = null;
            double base = 0.0;
            if (this.category != null) {
                Number b = ((DefaultCategoryDataset)dataset).getValue(series, 1);
                base = b.doubleValue();
            }
            else {
                base = calculateSeriesTotal(dataset, series);
            }
            Number value = dataset.getValue(series, 1);
            if (value != null) {
                double v = value.doubleValue();
                // you could apply some formatting here
                result = value.toString()
