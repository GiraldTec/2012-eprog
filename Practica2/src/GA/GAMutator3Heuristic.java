	...
	public Boolean mutate(IGAGene gen, double prob) {
		...
		if(IGARandom.getRDouble()<prob){
			...
			while( gSetIterador.hasNext() ){
				...
				double auxV;
				if ((auxV = auxgen.evaluate(this.students))<mejorV) {
					mejorGen = auxgen;	
					mejorV = auxV;
				}
			}
			gen.setGen(mejorGen.getGen());
			((GAStudentGene)gen).loadGene(students);
			res= !res;
		}
		return res;
	}
